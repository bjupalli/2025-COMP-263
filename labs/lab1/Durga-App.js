...
request.onsuccess = function (event) {
  ...
  function testSensorReadingsArray() {
    ...
    req.onsuccess = () => {
      ...
      console.assert(
        Array.isArray(record.sensorReadings),
        "SensorReadings should be an array"
      );

      console.assert(
        record.sensorReadings.length > 0,
        "SensorReadings should not be empty"
      );
  ...
  function testFarmerNoteString() {
    ...
    req.onsuccess = () => {
      ...
      console.assert(
        typeof record.farmerNote === "string",
        "FarmerNote should be a string"
      );

      console.assert(
        record.farmerNote.length > 0,
        "FarmerNote should not be empty"
      );
